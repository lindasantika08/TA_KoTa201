...
export default {
  ...
  computed: {
    ...
    totalScoreByAspect() {
        ...
        if (this.selectedUserData && this.selectedUserData.self_assessment) {
            this.selectedUserData.self_assessment.forEach(aspect => {
                let total = 0;
                let count = 0;

                aspect.questions.forEach(question => {
                    if (this.selectedScore.hasOwnProperty(question.pertanyaan)) {
                        total += this.selectedScore[question.pertanyaan] || 0;
                        count++;
                    }
                });

                aspectScores[aspect.aspek] = count > 0 ? total / count : 0;
            });
        }
  ...
  watch: {
    selectedUserData: {
        handler(newUserData) {
            if (newUserData) {
                // Inisialisasi ulang untuk self & peer assessment
                this.initializeSelectedScores(newUserData);
            }
        },
        immediate: true,
        deep: true // Pastikan perubahan dalam objek bersarang juga terdeteksi
    },
    selectedScore: {
        handler(newScores) {
            if (this.selectedUserData) {
                this.calculateAnalysisScores(this.selectedUserData);
            }
            // Pastikan newScores valid sebelum menyimpan ke localStorage
            if (newScores) {
                localStorage.setItem("selectedScore", JSON.stringify(newScores));
            }
        },
        deep: true
    }
  ...
  mounted() {
     // Ambil nilai selectedScore dari localStorage jika ada
    const savedScores = localStorage.getItem("selectedScore");
    if (savedScores) {
        this.selectedScore = JSON.parse(savedScores);
    } else {
        this.selectedScore = {}; // Pastikan tidak undefined
    }
  ...
  methods: {
    initializeSelectedScores(userData) {
  if (!userData) return;

  // Jika selectedScore belum ada (misalnya halaman pertama kali dibuka)
  if (!this.selectedScore) {
        this.selectedScore = {};
    }

  // Menangani Self Assessment
  if (userData.self_assessment) {
        userData.self_assessment.forEach(aspect => {
            if (aspect.questions && aspect.questions.length) {
                aspect.questions.forEach(question => {
                    // Hanya atur jika belum ada di selectedScore (supaya tidak overwrite)
                    if (!this.selectedScore.hasOwnProperty(question.pertanyaan)) {
                        this.selectedScore[question.pertanyaan] =
                            question.score !== undefined ? question.score : question.score_SLA;
                    }
                });
            }
        });
  ...
  // Menangani Peer Assessment
  if (userData.evaluated_by_peers && userData.evaluated_by_peers.length) {
        const groupedPeerEvaluations = this.groupPeerEvaluations(userData.evaluated_by_peers);
        groupedPeerEvaluations.forEach(group => {
            if (group.answers && group.answers.length) {
                group.answers.forEach(answer => {
                    if (!this.selectedScore.hasOwnProperty(answer.pertanyaan)) {
                        this.selectedScore[answer.pertanyaan] =
                            answer.score !== undefined ? answer.score : answer.score_SLA;
                    }
                });
            }
        });
    ...
    // Simpan kembali ke localStorage setelah inisialisasi
    ...
    async fetchKelompokAnalysis() {
      ...
      try {
        ...
        // If there's already a selected user, initialize their scores
        if (this.selectedUserId && this.userAnalysis[this.selectedUserId]) {
          this.initializeSelectedScores(this.userAnalysis[this.selectedUserId]);
        }
      } catch (error) {
      ...
      return userData.self_assessment.map((selfAspect) => {
        ...
        const averagePeerScore =
          matchingPeerEvaluations.length > 0
            ...
            const pertanyaanKeys = selfAspect.questions.map(q => q.pertanyaan);

    const selfScore = pertanyaanKeys.reduce((sum, key) => {
      return sum + (this.selectedScore[key] || 0);
    }, 0) / pertanyaanKeys.length;
                      ...
                      <div class="flex justify-between items-center mb-4">
                        ...
                        <div class="text-right">
                          ...
                          <div class="text-2xl font-bold" :class="{
                            ...
                            {{ (totalScoreByAspect[aspek.aspek] !== undefined ? totalScoreByAspect[aspek.aspek].toFixed(2) : 'N/A') }}
  ...
  qIndex
                              ...
                              <td class="px-4 py-3 text-center">
                                <label class="inline-flex items-center space-x-2 cursor-pointer">
                                  <input type="radio" v-model="selectedScore[pertanyaan.pertanyaan]"
                                    :value="pertanyaan.score" class="hidden" />
                                  <div
                                    class="w-6 h-6 border border-gray-400 flex items-center justify-center rounded cursor-pointer"
                                    :class="{ 'bg-blue-500 text-white': selectedScore[pertanyaan.pertanyaan] === pertanyaan.score }"
                                    @click="selectedScore[pertanyaan.pertanyaan] = pertanyaan.score">
                              ...
                              <td class="px-4 py-3 text-center">
                                <label class="inline-flex items-center space-x-2 cursor-pointer">
                                  <input type="radio" v-model="selectedScore[pertanyaan.pertanyaan]"
                                    :value="pertanyaan.score_SLA" class="hidden" />
                                  <div
                                    class="w-6 h-6 border border-gray-400 flex items-center justify-center rounded cursor-pointer"
                                    :class="{ 'bg-blue-500 text-white': selectedScore[pertanyaan.pertanyaan] === pertanyaan.score_SLA }"
                                    @click="selectedScore[pertanyaan.pertanyaan] = pertanyaan.score_SLA">
                                    {{ pertanyaan.score_SLA || 'N/A' }}
                                  </div>
                                </label>
                              </td>
...
answer, idx
                                ...
                                <td class="px-4 py-3 text-center">
                                  <label class="inline-flex items-center space-x-2 cursor-pointer">
                                    <input type="radio" v-model="selectedScore[answer.pertanyaan]" :value="answer.score"
                                      class="hidden" />
                                    <div
                                      class="w-6 h-6 border border-gray-400 flex items-center justify-center rounded cursor-pointer"
                                      :class="{ 'bg-blue-500 text-white': selectedScore[answer.pertanyaan] === answer.score }"
                                      @click="selectedScore[answer.pertanyaan] = answer.score">
                                      {{ answer.score || 'N/A' }}
                                    </div>
                                  </label>
                                </td>
                                <td class="px-4 py-3 text-center">
                                  <label class="inline-flex items-center space-x-2 cursor-pointer">
                                    <input type="radio" v-model="selectedScore[answer.pertanyaan]"
                                      :value="answer.score_SLA" class="hidden" />
                                    <div
                                      class="w-6 h-6 border border-gray-400 flex items-center justify-center rounded cursor-pointer"
                                      :class="{ 'bg-blue-500 text-white': selectedScore[answer.pertanyaan] === answer.score_SLA }"
                                      @click="selectedScore[answer.pertanyaan] = answer.score_SLA">
                                      {{ answer.score_SLA || 'N/A' }}
                                    </div>
                                  </label>
                                </td>
                                <!-- <td class="px-4 py-3 text-center">
...
score, index
  ...
  selectedUserData
                            ...
                            <td class="px-4 py-3 text-center">
                              <span class="inline-flex items-center px-3py-1 rounded-full text-xs font-medium" :class="{
                                'bg-green-100 text-green-800':
                                  parseFloat(
                    ...
                    <div class="mt-4 flex flex-wrap gap-4 text-xs text-gray-500">
                      <div class="flex items-center gap-2">
                        <span class="w-3 h-3 inline-block bg-green-100 rounded-full border border-green-200"></span>
                        <span>Over: Self score lebih
                          tinggi dari peer score</span>
                      </div>
                      <div class="flex items-center gap-2">
                        <span class="w-3 h-3 inline-block bg-yellow-100 rounded-full border border-yellow-200"></span>
                        <span>Match: Self score sama
                          dengan peer score</span>
                      </div>
                      <div class="flex items-center gap-2">
                        <span class="w-3 h-3 inline-block bg-red-100 rounded-full border border-red-200"></span>
