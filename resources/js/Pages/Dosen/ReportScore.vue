...
export default {
  ...
  methods: {
    ...
    calculateTotalScoreFinal(aspek) {
      if (!aspek.questions || !Array.isArray(aspek.questions) || aspek.questions.length === 0) {
        return 0;
      }
      ...
      try {
        ...
        this.selectedUserData.self_assessment.forEach((aspek) => {
          ...
          aspek.questions.forEach((pertanyaan) => {
            const finalScore = pertanyaan.final_score || pertanyaan.score;
        ...
        if (response.data.success) {
          ...
          alert(
            response.data.message || "Terjadi kesalahan saat menyimpan jawaban"
          );
        }
      } catch (error) {
    ...
    setupFinalScoresSelf() {
      ...
      this.selectedUserData.self_assessment.forEach((aspek) => {
        ...
        aspek.questions.forEach((pertanyaan) => {
          ...
          if (
            pertanyaan.report &&
            typeof pertanyaan.report.final_score_self !== "undefined"
          ) {
            pertanyaan.final_score = pertanyaan.report.final_score_self;
            console.log(
          ...
          this.selectedUserData.evaluated_by_peers.forEach((group) => {
            ...
            Object.entries(group.evaluated_by).forEach(
              ([peerId, evaluator]) => {
                ...
                const actualEvaluators = evaluator.evaluated_by
                  ? Object.entries(evaluator.evaluated_by)
                  : [[peerId, evaluator]];

                // Process each actual evaluator with its id
                actualEvaluators.forEach(([actualPeerId, actualEvaluator]) => {
                  if (
                    !actualEvaluator.answers ||
                    !Array.isArray(actualEvaluator.answers)
                  )
                    return;
                  ...
                  actualEvaluator.answers.forEach((answer) => {
                    ...
                    if (this._modifiedAnswers) {
                      const key =
                        answer.question_id + "_" + actualEvaluator.name;
                      const modifiedAnswer = this._modifiedAnswers.get(key);

                      if (
                        modifiedAnswer &&
                        modifiedAnswer.final_peer !== undefined
                      ) {
                        finalPeer = modifiedAnswer.final_peer;
                        console.log(
                    ...
                    if (finalPeer === null && answer.final_peer !== undefined) {
                      finalPeer = answer.final_peer;
                      console.log(
                    ...
                    else if (finalPeer === null && answer.score) {
                      finalPeer = answer.score;
                      console.log(
                    ...
                    if (finalPeer) {
                      ...
                      if (
                        this.selectedUserData.peer_assessment &&
                        Array.isArray(this.selectedUserData.peer_assessment)
                      ) {
                        for (const peerAspek of this.selectedUserData
                          .peer_assessment) {
                          // Cek jika aspek ini memiliki questions
                          if (
                            peerAspek.questions &&
                            Array.isArray(peerAspek.questions)
                          ) {
                            // Cari question yang matching dengan question_id
                            const matchingQuestion = peerAspek.questions.find(
                            ...
                            if (matchingQuestion) {
                              ...
                              if (peerAspek.kriteria_id) {
                                criteriaId = peerAspek.kriteria_id;
                              } else if (peerAspek.aspek) {
                                // Jika tidak ada kriteria_id, gunakan aspek sebagai fallback
                                criteriaId = peerAspek.aspek;
                              }
                              break;
                            }
                          }
                      ...
                      if (finalPeer !== null && finalPeer !== undefined) {
                        ...
                        answersPeer.push({
                          mahasiswa_id: mahasiswaId,
                          typeCriteria_id: criteriaId || "unknown_criteria",
                          question_id: answer.question_id,
                          peer_id: actualPeerId,
                          final_score_peer: Number(finalPeer),
                        });
                      }
                    }
                  });
      ...
      this.selectedUserData.self_assessment.forEach((aspek) => {
        ...
        aspek.questions.forEach((pertanyaan) => {
          ...
          if (pertanyaan.final_score_self) {
            pertanyaan.final_score = pertanyaan.final_score_self;
          } else if (pertanyaan.report && pertanyaan.report.final_score_self) {
            pertanyaan.final_score = pertanyaan.report.final_score_self;
          }
    ...
    setupFinalScoresPeer() {
      if (!this.selectedUserData || !this.selectedUserData.peer_assessment) {
        return;
      }

      this.selectedUserData.peer_assessment.forEach((peerGroup) => {
        if (!peerGroup.questions || !Array.isArray(peerGroup.questions)) {
          return;
        }
        ...
        peerGroup.questions.forEach((answer) => {
          ...
          } else if (answer.report && answer.report.final_score_peer) {
    ...
    calculateAnalysisScores(userData) {
      ...
      return userData.self_assessment.map((selfAspect) => {
        ...
        const matchingPeerEvaluations = userData.evaluated_by_peers.filter(
          (peer) =>
    ...
    prepareReportData(score) {
      return {
        ...
        nilai_total:
          (parseFloat(score.selfScore) + parseFloat(score.averagePeerScore)) /
          2,
        status: score.status,
      };
    },
      ...
      <main class="p-6">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mb-6">
          <Card title="Detail Kelompok" class="bg-white shadow-sm border-0">
            ...
            >
              <div class="flex items-center">
                <div class="w-36 font-medium text-gray-600">Tahun Ajaran:</div>
                <div>{{ batch_year }}</div>
              </div>
              <div class="flex items-center">
                <div class="w-36 font-medium text-gray-600">Nama Proyek:</div>
                <div>{{ project_name }}</div>
              </div>
              <div class="flex items-center">
                <div class="w-36 font-medium text-gray-600">Kelompok:</div>
                <div>{{ kelompok }}</div>
              </div>
            </div>
            <div v-else>
          ...
          >
            ...
            >
              ...
              <p class="text-sm text-gray-600">
                Menampilkan perbandingan skor evaluasi diri dengan rata-rata
                skor dari peer
              </p>
            </div>
          ...
          >
            ...
            <div class="p-4">
              <ApexChart
                ...
                height="400"
                :series="
                  prepareSelfComparisonChartData(selectedUserData).series
                "
                :options="
        ...
        >
          ...
          <h3 class="mt-4 text-lg font-medium text-gray-900">Pilih Peserta</h3>
                    ...
                    <div class="bg-gray-50 p-4 rounded-lg">
                      ...
                      <div class="flex justify-between items-center mb-4">
                        ...
                        <div class="text-right flex space-x-6">
                          ...
                          <div>
                            <div class="text-sm text-gray-600">Total Skor Asli</div>
                            <div
                              class="text-2xl font-bold"
                              :class="{
                                'text-green-600': aspek.total_score >= 4,
                                'text-yellow-600':
                                  aspek.total_score >= 3 && aspek.total_score < 4,
                          ...
                          <!-- Final Total Score -->
                          <div>
                            <div class="text-sm text-gray-600">Total Skor Final</div>
                            <div
                              class="text-2xl font-bold"
                              :class="{
                                'text-green-600': aspek.total_score_final >= 4,
                                'text-yellow-600':
                                  aspek.total_score_final >= 3 && aspek.total_score_final < 4,
                                'text-red-600': aspek.total_score_final < 2.5,
                              }"
                            >
                              {{
                                aspek.total_score_final
                                  ? aspek.total_score_final.toFixed(2)
                                  : aspek.total_score.toFixed(2)
                              }}
                            </div>
                          </div>
                                ...
                                <td class="px-4 py-3 text-center">
                                  ...
                                  >
                                    <input
                                      ...
                                      class="border-2 border-gray-300 rounded-md hover:border-blue-500"
                                      @change="
                                        updateFinalPeer(answer, answer.score)
                                      "
                                    />
                                    <span
                                      :class="{
