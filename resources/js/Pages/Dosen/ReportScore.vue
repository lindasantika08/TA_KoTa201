    ...
    methods: {
                ...
                Object.values(group.evaluated_by).forEach((evaluator) => {
                    ...
                    evaluators.forEach((actualEvaluator) => {
                        ...
                        actualEvaluator.answers.forEach((answer) => {
                            // Urutan fallback:
                            // 1. final_score_peer (hasil simpan dosen, bisa score atau score_SLA)
                            // 2. skor_peer (rata-rata peer)
                            // 3. score (nilai peer)
                            if (
                                answer.final_score_peer !== undefined &&
                                answer.final_score_peer !== null
                            ) {
                                answer.final_peer = answer.final_score_peer;
                            } else if (
                                answer.skor_peer !== undefined &&
                                answer.skor_peer !== null
                            ) {
                                answer.final_peer = answer.skor_peer;
                            } else if (
        ...
        calculateAnalysisScores(userData) {
            ...
            return userData.self_assessment.map((selfAspect) => {
                ...
                const selfScore =
                    selfAspect.total_score_final != null
                        ? selfAspect.total_score_final
