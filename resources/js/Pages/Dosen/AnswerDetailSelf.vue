...
export default {
  ...
  computed: {
    totalScore() {
      return this.answers.reduce((sum, answer) => sum + (parseFloat(answer.skor) || 0), 0);
    },
    averageScore() {
  ...
  methods: {
    fetchAnswerDetails() {
      ...
      axios.get('/api/answers/get-details', {
        params: {
          mahasiswaName: this.mahasiswaName,
    ...
    <div class="flex-1 bg-white shadow-sm">
      ...
      <main class="p-6 space-y-6">
        ...
        <div class="bg-white border rounded-lg p-6 shadow-sm">
          ...
          <div v-else-if="answers.length === 0" class="text-center text-gray-500 py-6">
          ...
          <div v-else>
            ...
            >
              ...
              <template #column-pertanyaan="{ item }">
                <div class="text-gray-800 font-medium">{{ item.pertanyaan }}</div>
              </template>

              <template #column-jawaban="{ item }">
                <div class="text-gray-600">{{ item.jawaban }}</div>
              </template>
              ...
              <template #column-skor="{ item }">
                <div class="text-center font-bold" 
                     :class="{
                       ...
                       'text-yellow-600': item.skor >= 5 && item.skor < 7,
