    ...
    return {
      ...
      feedback: {
        ...
        peerFeedback: [
          ...
          { 
            feedback: "Kontribusi yang bagus dalam tim. Aktif memberikan ide dan solusi.",
            createdAt: new Date(2025, 4, 1)
          },
          {
            feedback: "Sangat membantu dalam menyelesaikan tugas teknis yang rumit.",
    ...
    <div class="flex-1 flex flex-col">
      ...
      <main class="flex-1 p-6 md:p-8">
        <div class="mb-6">
          ...
          <p class="text-gray-600 mt-1">Pantau progres pembelajaran dan asesmen Anda</p>
        </div>
              ...
              <div class="flex items-center space-x-4">
                ...
                <div>
                  <span :class="`inline-flex items-center px-3 py-1 rounded-full text-sm font-medium ${statusClass}`">
                    <svg v-if="selfAssessmentStatus" class="w-4 h-4 mr-1.5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                      <path v-if="selfAssessmentStatus === 'Completed'" stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 13l4 4L19 7" />
                      <path v-else-if="selfAssessmentStatus === 'Pending'" stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z" />
              ...
              <div class="flex items-center space-x-4">
                ...
                <div class="flex-1">
                  <div class="flex justify-between items-center mb-1">
                    ...
                    <span class="text-sm font-medium text-gray-700">{{ peerCompletedCount }}/{{ peerGroupSize }}</span>
                  </div>
                  <div class="w-full bg-gray-200 rounded-full h-2.5">
                    <div 
                      :class="`h-2.5 rounded-full ${progressColor}`" 
                      :style="`width: ${peerGroupSize > 0 ? (peerCompletedCount / peerGroupSize * 100) : 0}%`"
                    ></div>
                  </div>
                </div>
              </div>
              ...
              <div class="flex flex-col space-y-4">
                <div class="flex items-center space-x-4">  
                  ...
                  <div class="w-full mt-2">
                    <label for="project-select" class="block text-sm font-medium text-gray-700 mb-1">Pilih Project:</label>
                    <select
                      id="project-select"
          ...
          <div class="lg:col-span-1">
            <div class="bg-white rounded-xl shadow-sm overflow-hidden h-full">
              <div class="px-6 py-5 border-b border-gray-100">
                ...
                <p class="text-sm text-gray-500 mt-1">Evaluasi dan saran untuk pengembangan Anda</p>
              </div>
              ...
              <div v-if="activeTab === 'peer'" class="p-4 max-h-64 overflow-y-auto">
                <div v-if="feedbackLoading" class="flex items-center justify-center h-32">
                  <div class="animate-spin rounded-full h-8 w-8 border-b-2 border-indigo-500"></div>
                </div>
                <div v-else-if="feedbackError" class="text-red-500 p-4 bg-red-50 rounded-lg">
                  {{ feedbackError }}
                </div>
                <div v-else-if="feedback?.peerFeedback?.length" class="space-y-3">
                  <div v-for="(item, index) in feedback.peerFeedback" 
                      :key="index" 
                      class="p-3 bg-blue-50 rounded-lg border border-blue-100">
                    <p class="text-sm text-gray-700">{{ item.feedback || "Peer memberikan penilaian positif terhadap kontribusi Anda." }}</p>
              ...
              <!-- Konten Tab Dosen Feedback -->
              <div v-if="activeTab === 'dosen'" class="p-4 max-h-64 overflow-y-auto">
                <div v-if="feedbackLoading" class="flex items-center justify-center h-32">
                  <div class="animate-spin rounded-full h-8 w-8 border-b-2 border-indigo-500"></div>
                </div>
                <div v-else-if="feedbackError" class="text-red-500 p-4 bg-red-50 rounded-lg">
                ...
                <div v-else-if="feedback?.lecturerFeedback?.length" class="space-y-3">
