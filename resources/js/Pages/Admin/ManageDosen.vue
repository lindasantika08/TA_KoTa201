    ...
    methods: {
        ...
        filterUsers() {
            ...
            this.filteredUsers = this.users.filter((user) => {
                ...
                const kodeDosenMatch = user.kode_dosen
                    .toLowerCase()
                    .includes(query);
                const nipMatch = user.nip.toLowerCase().includes(query);
                const emailMatch = user.user.email
                    .toLowerCase()
                    .includes(query);
                const majorMatch =
                    !this.selectedMajor ||
                ...
                return (
                    (nameMatch || kodeDosenMatch || nipMatch || emailMatch) &&
                    majorMatch
                );
            });
        },
        ...
        async deleteDosen(NIP) {
            let namaDosen = `NIP ${NIP}`;

            try {
                // Ambil data dosen dari server terlebih dahulu
                const response = await axios.get(`/api/name-dosen/${NIP}`);
                namaDosen = response.data.nama || response.data.name || `NIP ${NIP}`;
            } catch (error) {
                console.error('Error fetching dosen name:', error);
                // Tetap lanjut dengan namaDosen = NIP
            }

            // Tampilkan konfirmasi hanya sekali
            const result = await Swal.fire({
                title: `Yakin ingin menghapus dosen ${namaDosen}?`,
                text: `NIP: ${NIP}. Tindakan ini tidak dapat dibatalkan!`,
                icon: "warning",
            ...
            try {
                // Proses delete
                const deleteResponse = await axios.post("/api/delete-dosen", {
                    nip: NIP,
                });
                ...
                if (deleteResponse.status === 201 || deleteResponse.status === 200) {
                    Swal.fire({
                        icon: "success",
                        title: "Dihapus!",
                        text: `Dosen ${namaDosen} berhasil dihapus.`,
                    });
                    await this.fetchUsers();
                } else {
                    Swal.fire({
                        icon: "error",
                        title: "Gagal",
                        text: `Gagal menghapus dosen ${namaDosen}.`,
                    });
                }
            } catch (error) {
                ...
                if (error.response?.data?.message?.includes('Cannot delete')) {
                    Swal.fire({
                        icon: "error",
                        title: "Tidak dapat menghapus",
                        text: "Dosen masih memiliki data terkait yang harus dihapus terlebih dahulu.",
                    });
                    return;
                }

                // Cek apakah user masih ada setelah error
                await this.fetchUsers();
                ...
                if (!userStillExists) {
                    Swal.fire({
                        icon: "success",
                        title: "Dihapus!",
                        text: `Dosen ${namaDosen} berhasil dihapus meskipun terjadi kesalahan.`,
                    });
                } else {
                    Swal.fire({
                        icon: "error",
                ...
                <div class="mb-6">
                    <div class="mb-6">
                        ...
                        <p class="text-gray-600">
                            Manage dosen data and their details
                        </p>
                    </div>
                </div>
                    ...
                    <template v-else-if="filteredUsers && filteredUsers.length">
                        <div class="flex justify-between items-center mb-4 px-4">
                            <div v-if="searchQuery" class="text-sm text-gray-600">
                                Found {{ filteredUsers.length }} result(s) for
                                "{{ searchQuery }}"
                            </div>
