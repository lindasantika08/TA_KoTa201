    ...
    methods: {
        ...
        async fetchDropdownOptions() {
            try {
                ...
                if (this.combinedOptions.length > 0) {
                    await this.initializeSelectedProject();

                    // If no project was loaded from localStorage, select the first available project
                    if (
                        !this.selectedProject &&
                        this.combinedOptions.length > 0
                    ) {
                        const firstOption = this.combinedOptions[0];
                        this.selectedOption = firstOption;
                        this.selectedProject = {
                            batch_year: firstOption.batchYear,
                            project_name: firstOption.projectName,
                        };
                        ...
                        // Fetch statistics for the first project
                        await this.fetchStatistics();
                        await this.fetchPeerStatistics();
                    }
                }
            } catch (error) {
        ...
        async fetchPeerStatistics() {
            if (!this.selectedProject) return;

            try {
                const response = await axios.get(
                    "/sispa/api/answers/statistics-peer",
                    {
                        params: {
        ...
        async fetchStatistics() {
            ...
            try {
                const response = await axios.get(
                    "/sispa/api/answers/statistics-dashboard",
                    {
                        params: {
                            batch_year: this.selectedProject.batch_year,
                            project_name: this.selectedProject.project_name,
